...
export default {
  ...
  actions: {
    getrecommendlist(context) {
      request.get('http://localhost:27952/api/server/content/index.json').then(data => {
        context.commit('setrecommendlist', data.data.recommendPage.list),
          context.commit('setBannerList', data.data.slideList.splice(0, 5))
      })
    },
    getgymList(context) {
      request.get('http://localhost:27952/api/server/content/moreProductPlay.json?fcity=-1&pageNum=1&type=1').then(data => {
